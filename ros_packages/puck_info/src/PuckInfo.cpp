...
void PuckInfo::imageCallback(const sensor_msgs::ImageConstPtr& msg) {
    ...
    for (auto& color : colors_) {
        std::cout << color.min << std::endl;
        std::cout << color.max << std::endl;
    }
}
...
void PuckInfo::initColorVector()
{
    ...
    colors_.emplace_back(Color(Scalar(color_min_from_param[0], color_min_from_param[1], color_min_from_param[2]),
                         Scalar(color_max_from_param[0], color_max_from_param[1], color_max_from_param[2]),
                         Color_e::yellow));
                              ...
                              Point &mask_centroid_point, int &mask_centroid_index)
{
    ...
    switch (color_index){
        ...
        case Color_e::green:
            ...
            break;
        case Color_e::red:
            circle(frame_, Point((int)(centroids.at<double>(region_label_index, 0)), (int)(centroids.at<double>(region_label_index, 1))), 5, Scalar(0, 0, 255), 5);
            break;
        default:
...
{
    cv::inRange(hsv, min_color, max_color, mask);
                                                   ...
                                                   const Scalar &max_color, Color_e color_index)
{
    ...
    if(debug_mode_)
    {
        ...
        win_name = win_name + color_to_string(color_index);
...
Centroid PuckInfo::findPuck()
{
    ...
    if (frame_.channels() > 2) {
        ...
        return Centroid(Color_e::none, Point(-1, -1), -1);
    }
