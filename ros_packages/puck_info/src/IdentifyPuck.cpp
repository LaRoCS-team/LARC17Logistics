...
IdentifyPuck::IdentifyPuck(): loopRate(2)
{
  DEBUG = false;

  image_transport::ImageTransport imgTrans(node);
  imgSub = imgTrans.subscribe("image_raw", 2, &IdentifyPuck::imageCallback, this);
  actionIdSub = node.subscribe("action_id", 2, &IdentifyPuck::actionIdCallback, this);

  pub = node.advertise<robotino_msgs::PuckInfo>("identifyPuck", 10);

  image_transport::ImageTransport it(node);
  teste_pub_ = it.advertise("tamo_sofrendo", 1);
}

void IdentifyPuck::imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  {
    ROS_ERROR("IdentifyPuck: cv_bridge exception: %s", e.what());
    return;
  }
...
void IdentifyPuck::actionIdCallback(const std_msgs::UInt64::ConstPtr& msg)
{
    action_id_ = msg->data;
}

// Initialize the range of all three the detectable colors
vector<Scalar> IdentifyPuck::initColorVector(vector<Scalar> colors)
{
  // NO Color
...
IdentifyPuck::filterCentroid(Mat frame, Mat mask, Mat labels, Mat status, Mat centroids, int i)
{
  int regionArea = 0;
...
// on the mask
Mat IdentifyPuck::getTreatedInRangeHSV(Mat hsv, Scalar minColor, Scalar maxColor)
{
  Mat treatedMask;
...
pair<int, pair<Point, int> > IdentifyPuck::detectColor(Mat frame, Mat hsv, Scalar minColor, Scalar maxColor, int colorIndex)
{
...
pair<int, Point> IdentifyPuck::findPuck(Mat frame, vector<Scalar> colors, int desiredColor)
{
  Mat hsv;
